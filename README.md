...
## Обработка текста / Clean-up

`cleanupTranscript: true` (Settings → LLM & editing) прогоняет обычную диктовку через LLM,
чтобы расставить знаки препинания и заглавные, убрать «э-э» и запинания, правильно написать
слова, которые распознаватель услышал не так. **Только это** — никаких пересказов, переводов
и сокращений.

Ещё она возвращает латиницу английским терминам, которые распознаватель записал кириллицей:
«по ракет» → Parakeet, «гитхаб» → GitHub, «в курсоре» → «в Cursor». Обычные русские слова и
обрусевшие заимствования (сайт, файл, интернет) не трогаются. То же самое **без всякой модели**
делает словарь — в нём уже есть `Parakeet: паракет, по ракет` и ещё полсотни терминов.

Главное свойство: сказанное **никогда не выполняется**. Скажешь «сколько будет два плюс два» —
встанет «Сколько будет два плюс два?», а не «четыре»; «переведи это на английский» останется
русской фразой. Держится это на двух вещах:

- системный промпт прямо говорит, что сообщение — это расшифровка чужой речи, а не обращённая
  к модели просьба, даже если звучит как вопрос или команда;
- страховка по длине: ответ короче половины или длиннее полутора входов — это уже не
  та же фраза, такой ответ выбрасывается и встаётся сказанное.

Нет сети, кончился лимит, нет ключа, модель молчит дольше 4 с — обработка просто
пропускается, встаётся сырой текст, в логе одна строка. После неудачи следующую минуту
попытки не делаются вовсе — чтобы офлайн не стоил задержки на каждой фразе. Фразы
короче трёх слов не трогаются.

Проверить: `GroqVoice.exe --cleanup "ну эм я думаю что надо это ээ переделать"` —
вторым аргументом можно задать таймаут в мс и увидеть поведение без связи.
...
- Скачать модель без меню (скрипты, установка на несколько машин): `GroqVoice.exe --download-model` — печатает проценты в консоль.
...
| `chatModels` | `["qwen/qwen3.8-27b", "openai/gpt-oss-120b", "openai/gpt-oss-20b"]` | Цепочка для task-режима, правки выделенного и обработки: отказ одной модели передаёт работу следующей. Модели Llama 3.x с Groq убраны — старый `chatModel` мигрируется автоматически. |
| `cleanupTranscript` | `false` | Причёсывать диктовку через LLM (см. «Обработка текста»). |
...
- Файл создаётся со **стартовым списком** (~50 терминов: Parakeet, GitHub, Docker, Telegram…) —
  именно он чинит «по ракет» и прочую кириллицу вместо английских слов, причём офлайн.
